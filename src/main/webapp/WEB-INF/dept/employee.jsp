...
<form action="" method="post">
	...
	<center>
		...
		<a href="${pageContext.request.contextPath }/findEmployee.action?pageNo=1">首页</a>
		<c:if test="${pageList.pageNo>1 }">
			<a href="${pageContext.request.contextPath }/findEmployee.action?pageNo=${pageList.pageNo-1 }">上一页</a>
		</c:if>
		<!-- 计算begin，end -->
		<c:choose>
		...
		<c:forEach var="i" begin="${begin }" end="${end }">
			<c:choose>
				...
				<c:otherwise>
					<a href="${pageContext.request.contextPath }/findEmployee.action?pageNo=${i }">[${i }]</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		
		<c:if test="${pageList.pageNo<pageList.pages }">
			<a href="${pageContext.request.contextPath }/findEmployee.action?pageNo=${pageList.pageNo+1 }">下一页</a>
		</c:if>
	
		<a href="${pageContext.request.contextPath }/findEmployee.action?pageNo=${pageList.pages }">尾页</a>		
	</center> 
</form>
</body>
</html>